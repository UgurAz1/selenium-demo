...
const authFile = path.resolve("test-data/user.json");
...
export class UserHelper {
  static save(user: TestUser) {
    try {
      ...
      throw new Error(`[UserHelper.save] Failed to write file:\n${(err as Error).message}`);
    }
  }
  ...
  static load(): TestUser {
    ...
    try {
      ...
      throw new Error(`[UserHelper.load] Failed to parse JSON:\n${(err as Error).message}`);
    }
  }
